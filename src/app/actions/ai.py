...
@register_action("ai_summarize")
async def action_ai_summarize(
    params: dict[str, Any],
    context: dict[str, Any]
) -> dict[str, Any]:
    """
    テキストを要約する（ショートカットアクション）

    params:
        text: 要約するテキスト (必須)
        max_length: 最大文字数 (デフォルト: 200)
        provider: "gemini" のみ (デフォルト: gemini)
        language: 出力言語 (デフォルト: japanese)

    Returns:
        ai_generate と同じ形式 + "summary"
    """
    ...
    prompt = f"""以下のテキストを{language}で要約してください。最大{max_length}文字程度で、要点を箇条書きでまとめてください。

{text}"""

    result = _call_gemini(prompt, model, api_key)
