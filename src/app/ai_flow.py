...
def _build_system_prompt() -> str:
    return (
        ...
        "\n"
        "パラメータ設定のガイドライン:\n"
        "- 各アクションのパラメータには、説明文に「※AI使用時・初心者はこちらを優先」と記載されているものがあります。\n"
        "- このマークが付いているパラメータを必ず優先的に使用してください（シンプル形式）。\n"
        "- 例: properties_simple を properties より優先、filter_simple を filter より優先、sorts_simple を sorts より優先。\n"
        "- シンプル形式は初心者にも分かりやすく、値の型から自動で適切な形式に変換されます。\n"
        "- 「上級者向け」と記載されているパラメータは、特別な理由がない限り使用しないでください。\n"
    )
...
def _build_params_system_prompt() -> str:
    return (
        ...
        "- このマークが付いているパラメータを必ず優先的に使用してください（シンプル形式）。\n"
        "- 例: properties_simple を properties より優先、filter_simple を filter より優先、sorts_simple を sorts より優先。\n"
        "- シンプル形式は初心者にも分かりやすく、値の型（文字列、数値、日付、ブール値など）から自動で適切な形式に変換されます。\n"
        "- 「上級者向け」と記載されているパラメータは、特別な理由がない限り使用しないでください。\n\n"
        "## テンプレート変数\n"
        "- {{ run_id }}: 実行ID\n"
        "- {{ now }}: 現在時刻（ISO8601）\n"
