...
## 更新履歴
- 2026-03-22: AI スキル機能を追加（`skills/<名>/SKILL.md` を `ai_generate` のシステムプロンプトに統合）。ドキュメント・Help・API バージョン 0.2.0・Docker Compose で `skills` をマウント。
- 2026-01-19: フローエディタの高さ計算をドキュメント位置基準に変更し、ナビ表示タイミング/スクロール復元によるリロード時の高さズレを抑制。
- 2026-01-19: フローエディタのパネル高再計算をフォント読み込み/レイアウト変化に追随させ、ハードリロード時の高さズレを抑制。
- 2026-01-19: 戻る操作でページ遷移中クラスが残る問題を解消し、dashboardの操作不能を防止。
- 2026-01-19: フローエディタの高さ倍率を1.5に再設定。
- 2026-01-19: モバイル/狭幅時の最小高さ固定を緩和し、フローエディタの高さ変更が反映されるよう調整。
- 2026-01-19: フローエディタの最小パネル高を倍率に合わせてスケールし、倍率変更が反映されるように調整。
- 2026-01-19: フローエディタの高さ倍率を0.8に再調整。
- 2026-01-19: フローエディタの高さ倍率を0.5に変更し、flow-editorの静的ファイルをno-cache配信に調整。
- 2026-01-19: フローエディタのCSS/JSを更新時刻でキャッシュバスターし、再読み込みで高さ調整が反映されるよう改善。
- 2026-01-19: フローエディタの下部パネル高さを1.5倍に拡大。
- 2026-01-19: フローエディタの下部パネル高さを安定化し、ノード間ラインが下部でも欠けないようキャンバス高を自動拡張するよう調整。
- 2026-01-19: フロー編集の削除ボタンをホバー時に小さく表示するように調整。
- 2026-01-19: 先頭が ai_generate のときだけ起動モーダルを表示し、手動実行時に一時プロンプトを渡せるようにした。
