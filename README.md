...
- **AI スキル**: `skills/<名前>/SKILL.md` を読み込み、`ai_generate` のシステムプロンプトに統合
...
# 2) コンテナ起動
docker run -d --name orbit-app -p 8000:8000 --env-file .env -v "${PWD}/workflows:/app/workflows" -v "${PWD}/skills:/app/skills" -v "${PWD}/runs:/app/runs" -v "${PWD}/secrets:/app/secrets" -v "${PWD}/backups:/app/backups" orbit:latest
```
...
| アクション名 | 説明 |
|------------|------|
| `ai_generate` | AI によるテキスト生成（Gemini）、Web検索対応、`skills` でローカルスキル適用可 |
| `ai_judge` | AI による yes/no 判定（Gemini） |

#### AI スキル（`ai_generate` の `skills` パラメータ）

プロジェクト直下の `skills/<スキル名>/SKILL.md` を読み込み、YAML フロントマター（任意）と Markdown 本文をシステムプロンプトに連結します。ビジュアルエディタではパラメータ `skills` を選ぶと **チェックボックス**で利用スキルを選べます。YAML を直接編集する場合は、カンマ区切り文字列または配列でも指定できます。
...
steps:
  - id: gen_1
    type: ai_generate
    params:
      prompt: "次の文章を要約してください: {{ step_1.text }}"
      skills: "summarizer, json_output"
      # または:
      # skills: ["summarizer", "json_output"]
```

既存の `system` パラメータがある場合、スキル本文のあとに付与されます。
...
| `GEMINI_MODEL` | Gemini モデル名（指定箇所は .env のみ） | ai_generate / ai_judge / AI生成使用時 |
